...
# Legacy (não incluído na Phase 2 atual)
...
def test_retrieval_comparison_llm_judge(capsys, db_ready, require_openai, llm_judge_yesno):
    if os.getenv("RAG_PHASE2_LEGACY", "").strip().lower() not in ("1", "true", "yes", "on"):
        pytest.skip("Teste legacy da Fase 2 desativado (defina RAG_PHASE2_LEGACY=1)")
    """Compara search_type (vector/text/hybrid_rrf) com LLM-judge, fixando o melhor chunking disponível.
    ...
    empresa = "Empresa X"
    chunking = _choose_best_chunking(db_ready, empresa)
    ...
    combos = ["vector", "text", "hybrid_rrf"]
    results = []
    for search_type in combos:
        hits = 0
        llm_yes = 0
        total = 0
        for q in queries:
        ...
        results.append((search_type, round(hits / max(total, 1), 2), round(llm_yes / max(total, 1), 2)))

    print("\n=== COMPARAÇÃO (search_type) — chunking fixo ===")
    print("search     | hit@5 | llm_yes")
    print("-" * 34)
    for search_type, hitk, lyes in results:
        print(f"{search_type:9s}| {hitk:5.2f} | {lyes:7.2f}")

    assert any(hitk >= 0.5 for _, hitk, _ in results)
