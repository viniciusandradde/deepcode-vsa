...
CREATE TABLE IF NOT EXISTS planning_projects (
    ...
    empresa TEXT,                  -- multi-tenancy (same as kb_docs)
    client_id UUID,               -- multi-tenancy (same as kb_docs)
    linear_project_id TEXT,       -- Linear.app project ID if synced
    linear_project_url TEXT,      -- Linear.app project URL
...
CREATE TABLE IF NOT EXISTS planning_stages (
    ...
    start_date DATE,
    end_date DATE,
    linear_milestone_id TEXT,       -- Linear.app milestone ID if synced
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS planning_documents (
    ...
    content TEXT,                   -- texto extraído para o LLM analisar
    file_size INTEGER,
...
CREATE TABLE IF NOT EXISTS planning_budget_items (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    project_id UUID NOT NULL REFERENCES planning_projects(id) ON DELETE CASCADE,
    stage_id UUID REFERENCES planning_stages(id) ON DELETE SET NULL,
    category TEXT NOT NULL,         -- infra, pessoal, licencas, hardware, software, servicos
    description TEXT,
...
CREATE TRIGGER trg_planning_projects_updated
    BEFORE UPDATE ON planning_projects
    FOR EACH ROW EXECUTE FUNCTION update_planning_updated_at();

DROP TRIGGER IF EXISTS trg_planning_stages_updated ON planning_stages;
CREATE TRIGGER trg_planning_stages_updated
    BEFORE UPDATE ON planning_stages
    FOR EACH ROW EXECUTE FUNCTION update_planning_updated_at();

DROP TRIGGER IF EXISTS trg_planning_budget_updated ON planning_budget_items;
CREATE TRIGGER trg_planning_budget_updated
    BEFORE UPDATE ON planning_budget_items
    FOR EACH ROW EXECUTE FUNCTION update_planning_updated_at();
...
CREATE OR REPLACE FUNCTION get_planning_project_summary(p_project_id UUID)
RETURNS TABLE (
    project_id UUID,
    title TEXT,
    status TEXT,
...
BEGIN
    ...
    SELECT 
        ...
        p.title,
        p.status,
        (SELECT COUNT(*)::INTEGER FROM planning_stages s WHERE s.project_id = p.id) AS total_stages,
        (SELECT COUNT(*)::INTEGER FROM planning_stages s WHERE s.project_id = p.id AND s.status = 'completed') AS completed_stages,
        (SELECT COUNT(*)::INTEGER FROM planning_documents d WHERE d.project_id = p.id) AS total_documents,
        (SELECT COALESCE(SUM(b.estimated_cost), 0) FROM planning_budget_items b WHERE b.project_id = p.id) AS total_budget_estimated,
        (SELECT COALESCE(SUM(b.actual_cost), 0) FROM planning_budget_items b WHERE b.project_id = p.id) AS total_budget_actual
...
CREATE OR REPLACE FUNCTION get_planning_documents_context(p_project_id UUID)
RETURNS TEXT AS $$
DECLARE
    result TEXT := '';
    doc RECORD;
